...
const rowHTML = (r) => `<button class="brow ${r.status}" data-id="${r.id}" aria-label="${esc(r.uni.short)}: ${STATUS[r.status]}">
  ...
  <span class="flap sc ${r.status !== 'fit' ? 'dim' : ''}">${r.score == null ? '—' : r.score}</span>
</button>`;

export function boardHTML(results, { title = 'DEPARTURES · ВУЗЫ', skeleton = false, foot = '' } = {}) {
  const rows = skeleton
    ? Array.from({ length: 6 }, (_, i) => `<div class="brow skeleton"><span class="code">········</span><span class="name">··········</span><span class="meta">·······</span><span class="stc"><span class="st">·······</span></span><span class="sc">··</span></div>`).join('')
    : results.map(rowHTML).join('');
  return `<div class="board" aria-live="polite">
    <div class="board-head"><span class="t">${title}</span><span class="clock" data-clock>${clock()}</span></div>
...
export function updateBoard(board, results, { foot = '' } = {}) {
  ...
  results.forEach((r) => {
    ...
    if (!el) { const t = document.createElement('div'); t.innerHTML = rowHTML(r); el = t.firstElementChild; }
